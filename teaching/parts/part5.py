...
ax.axhline(tci_ts["TCI"].mean(), color="#bc4749", ls="--", lw=1.2,
           label=f"mean = {tci_ts['TCI'].mean():.1f}%")
ax.set_title("TVP-VAR total connectedness index: system integration varies over time")
ax.set_ylabel("TCI (%)")
ax.legend()
fig.tight_layout()
plt.show()
...
for k, (col, c) in enumerate(zip(sub.columns, ["#1b4965", "#f4a261", "#2a9d8f"])):
    ax.bar(x + (k-1)*w, sub[col], width=w, label=f"{col} days", color=c)
ax.set_xticks(x); ax.set_xticklabels(sub.index, rotation=45, ha="right")
ax.axhline(0, color="grey", lw=0.8)
ax.set_ylabel("Net spillover")
ax.set_title("Net spillover by band: short horizons dominate, long ones vanish")
ax.legend()
fig.tight_layout()
plt.show()
