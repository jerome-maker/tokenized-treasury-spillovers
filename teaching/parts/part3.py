...
ax.plot(r_series.index, r_series.abs(), lw=0.4, color="#c9c9c9", label="$|r_t|$ (realised)")
ax.plot(cond_vol.index, cond_vol, lw=1.3, color="#bc4749", label=r"$\\sigma_t$ (GARCH conditional vol.)")
ax.set_title("GARCH(1,1) conditional volatility: clustering distilled into a smooth path")
...
ax.plot(e, nic_garch(e, 0.05, 0.10, 0.85, s2bar), lw=1.8, color="#1b4965",
        label=r"GARCH: symmetric ($\\gamma=0$)")
ax.plot(e, nic_gjr(e, 0.05, 0.05, 0.12, 0.85, s2bar), lw=1.8, color="#bc4749",
        label=r"GJR: $\\gamma=+0.12$ (leverage effect, equity pattern)")
ax.plot(e, nic_gjr(e, 0.05, 0.16, -0.12, 0.85, s2bar), lw=1.8, color="#2a9d8f", ls="--",
        label=r"GJR: $\\gamma=-0.12$ (reversed, flow-driven pattern)")
ax.axvline(0, color="grey", lw=0.8)
ax.set_xlabel(r"Prior shock $\\varepsilon_{t-1}$")
ax.set_ylabel(r"$\\sigma_t^2$")
ax.set_title("News impact curve: the sign of $\\gamma$ decides which way it tilts")
